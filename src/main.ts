...
const icon = (name: keyof typeof icons, size = 18) => `<svg width="${size}" height="${size}" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">${icons[name]}</svg>`;
...
function updateUI(): void {
  ...
  $('#traffic-flow').textContent = Math.round(metrics.flow).toLocaleString('en-CH');
  ...
  if (phase !== previousPhase) {
    ...
    release.innerHTML = phase === 'blocking' ? `Clear the left lane ${icon('arrow')}` : phase === 'overtaking' ? 'Overtaking…' : `Left lane released ${icon('check')}`;
    $('#road-status').textContent = phase === 'blocking' ? 'Left lane blocked' : phase === 'overtaking' ? 'Finding a safe gap' : 'Blocker moved right';
    $('.road-badge').classList.toggle('is-clear', phase === 'clear');
    $('#action-title').textContent = phase === 'blocking' ? 'Give traffic a little room.' : phase === 'overtaking' ? 'A safe pass takes a moment.' : 'Room to move again.';
    $('#action-description').textContent = phase === 'blocking' ? 'Let the orange car finish overtaking and move back to the right.' : phase === 'overtaking' ? 'The driver is accelerating and looking for a safe gap on the right.' : 'Watch the cars behind accelerate. Restart to run the experiment again.';
  }
  drawChart(chart, sim);
}
...
function applySettings(): void {
  ...
  $('#below-limit').textContent = `${settings.speedLimit - settings.blockerSpeed} below limit`;
  $('#density-name').textContent = settings.vehicleCount < 22 ? 'Light' : settings.vehicleCount > 32 ? 'Heavy' : 'Moderate';
...
$('#reset').addEventListener('click', () => { sim.reset(); updateUI(); });
$('#release').addEventListener('click', () => { sim.release(); updateUI(); });
$('#show-speeds').addEventListener('change', event => { renderer.showSpeeds = (event.target as HTMLInputElement).checked; });
document.querySelectorAll<HTMLButtonElement>('[data-speed]').forEach(button => button.addEventListener('click', () => {
  ...
  document.querySelectorAll<HTMLButtonElement>('[data-speed]').forEach(other => {
    other.classList.toggle('selected', other === button);
    other.setAttribute('aria-pressed', String(other === button));
  });
}));
const dialog = $<HTMLDialogElement>('#about-dialog');
document.querySelectorAll('.about-trigger').forEach(button => button.addEventListener('click', () => dialog.showModal()));
...
function frame(now: number): void {
  ...
  renderer.draw(sim);
  if (now - previousUI > 150) { updateUI(); previousUI = now; }
