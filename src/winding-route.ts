...
type Segment =
  | { kind: "line"; x: number; y: number; angle: number; length: number }
  | {
      kind: "arc";
      x: number;
      y: number;
...
export class WindingRoute {
  ...
  constructor(width: number, height: number) {
    this.roadWidth = Math.min(width < 600 ? 24 : 32, height * 0.07);
    const top = Math.min(58, height * 0.12);
    const bottom = height - Math.min(115, height * 0.22);
    this.rows = Math.max(
      4,
      Math.min(8, Math.floor((bottom - top) / (width < 600 ? 82 : 105)) + 1),
    );
  ...
  pose(distance: number, offset = 0): RoadPose {
    ...
    for (const segment of this.segments) {
      ...
      if (segment.kind === "line") {
        ...
        const radial =
          segment.start + (segment.sweep * remaining) / segment.length;
