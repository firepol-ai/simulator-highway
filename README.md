...
The original road is a deterministic, repeating 1.2 km loop with a fixed number of vehicles. Cars are enlarged for readability; the visualization is schematic. The original mode follows the blocker in a closer view, drawing car lengths to the distance scale so bumper gaps remain visible. Winding mode shows the entire 6 km circuit; its bends are a visual layout and do not impose cornering speed limits. Winding-mode vehicle sizes are schematic. A 120 km/h vehicle travels 20% farther per second than one at 100 km/h; the longer circuit takes more time to traverse. At 400–800 vehicles the 6 km circuit can become congested throughout, so use a lower density to observe a distinct queue grow behind the blocker. This is an illustrative model, not a calibrated forecast or a complete implementation of traffic law. Desired speeds above the chosen limit represent driver behavior, not advice. The right-lane slow-driver control sets the trucks’ desired speed (108 km/h by default), independently of the general limit. Trucks can still be slowed by vehicles ahead. Cars more than 8 km/h below their desired speed count as held back. Flow is estimated as vehicle density multiplied by average speed; it is not detector-measured throughput.
