...
- **Impatient right-side passing:** only held-back faster drivers try the right lane. After a short delay they frequently take safe openings, estimating whether there is room to accelerate past their target before catching right-lane traffic. A slower truck farther ahead does not rule out a pass when the gap is large enough. With this option or signalling enabled, faster drivers follow at short bumper gaps (roughly 2–4 m once speeds match), retaining closing-speed braking and collision separation. They use shorter merge gaps, then move back left only after fully clearing the car they were following. Available space and slower right-lane traffic affect whether a pass can succeed.
- **Horns & flashing lights:** only frustrated faster drivers in the left lane signal the car ahead. They signal only within 10 m of the car ahead. They flash three times, spaced two simulated seconds apart, before using the horn. Some ordinary, limit-respecting drivers respond to the first flashes; others wait for the horn. They move right only through a safe gap. The blocker ignores these signals until you select Clear the left lane. Drivers cruising on the right are left alone. Headlight beams show the flashes; separate horn rings appear when honking, even with sound muted.
...
Vehicles follow an illustrative model with separate free-speed and following constraints, acceleration limits, closing-speed-dependent headways, and gap checks for lane changes. Ordinary drivers also return right to give approaching faster traffic room when the next right-lane vehicle is sufficiently far ahead. Ordinary cars target the general speed limit; a third of the cars in each initial lane target the faster-driver setting. Distant traffic does not lower a car’s cruising target; cars slow as they approach a leader or its queue. Ordinary cars target short 4–6 m bumper gaps at matched speeds; impatient cars follow closer. Braking depends on the relative closing speed, and collision checks account for the leader’s movement. Adding vehicles does not lower their configured targets: uniformly spaced traffic can maintain 120 km/h even at 800 vehicles when nothing ahead is slower. Faster drivers overtake left and return right. A virtual leader discourages passing slower left-lane traffic on the right. The blocking driver remains left until released.

The straight view supports 12–800 vehicles. Its loop stays at 1.2 km through 160 vehicles, then grows with the count to 6 km at 800 so cars fit without overlapping; the displayed loop length updates with the setting. Each experiment keeps a fixed number of vehicles. Cars are enlarged for readability; the visualization is schematic. The original mode follows the blocker in a closer view, drawing car lengths to the distance scale so bumper gaps remain visible. Road markings and roadside trees scroll with the following camera; car motion in this view is relative to the blocker. Winding mode shows the entire 6 km circuit; its bends are a visual layout and do not impose cornering speed limits. Winding-mode vehicle sizes are schematic. A 120 km/h vehicle travels 20% farther per second than one at 100 km/h; the longer circuit takes more time to traverse. At high density, a queue caused by slower traffic can spread through both lanes. This is an illustrative model, not a calibrated forecast or a complete implementation of traffic law. Desired speeds above the chosen limit represent driver behavior, not advice. The right-lane slow-driver control sets the trucks’ desired speed (108 km/h by default), independently of the general limit. Trucks can still be slowed by vehicles ahead. Cars more than 8 km/h below their desired speed count as held back. Flow is estimated as vehicle density multiplied by average speed; it is not detector-measured throughput.
